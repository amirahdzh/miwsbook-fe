<template>
    <div v-if="isVisible" class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-75 z-50">
      <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg">
        <button @click="close" class="absolute top-4 right-4 text-gray-500 hover:text-gray-800">
          <Icon icon="mdi:close" />
        </button>
        ...
        <button @click="searchBooks" class="bg-primary text-white font-bold px-4 py-2 rounded-full hover:bg-teal-600">
          Search
        </button>
  
        <ul class="mt-4">
          <li v-for="book in filteredBooks" :key="book.id" class="border-b py-2">
            <a @click="selectBook(book)" class="cursor-pointer hover:text-teal-600">{{ book.title }}</a>
          </li>
        </ul>
      </div>
    </div>
  ...
  import { getBooks } from '../services/bookService'; // Assuming you have a service for fetching books
